...
parser.add_argument("output_dir", nargs='?', default="/home/m/ws/data/fork13/", help="Path to where the data should be saved")
...
files = os.listdir(args.data_dir)[:20000]
output_number = 0
...
for f in files:
    if 'json' in f:
        ...
        # for objkey in bpy.context.scene.objects.keys():
        #     if 'wall' in bpy.context.scene.objects[objkey].name.lower():
        #         bpy.context.scene.objects[objkey].select_set(True)
        ...
        bproc.renderer.enable_segmentation_output(map_by=["category_id", "instance", "name", "class"], default_values={'category_id': 0})
